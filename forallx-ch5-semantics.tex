...
HUN: Ez azt jelenti, hogy $a$ bármilyen mondatot felvesz SL-ből és hozzáköt egy egyest vagy egy nullát; egy, ha a mondat igaz és 0, ha a mondat hamis. $a$ függvény részleteinek jelentését a mondat betűi együtt határozzák meg a szavak állításaival együtt. Ha $D$ jelentése `Sötét van odakinnt`, akkor $a(D)=1$ este egy nagy vihar alkalmával, miközben $a(D)=0$ egy derűs napot jelent.
...
HUN: $a$-ra gondolhatunk úgy is, mintha egy igazságtábla egy sora lenne. Ahol az igazságtábla sora hozzáköt egy igazságértéket néhány atomi mondathoz, az igazságérték feladata hozzákötni egy értéket minden atomi SL mondathoz. Végtelenül sok mondat betű van, ahol az igazságváltozó feladata, hogy mindenhová értéket rendeljen. Amikor egy igazságtáblát szerkesztünk, a mondatnak csak azon betűivel foglalkozunk, amik befolyásolják a mondat igazságtartalmát (értékét) és mint olyan, a többi részével nem foglalkozunk. Tulajdonképpen minden sora az igazságtáblának egy \emph{partial} igazságérték hozzárendelést ad.
...
HUN: Fontos megjegyezni, hogy az $a$ igazságérték hozzárendelés nem része SL nyelvezetének, ez inkább egy matematikai gépezet része, amit az SL leírásához, jellemzéséhez használunk. Azt kódolja, hogy az atomi mondatnak mely része igaz vagy hamis.

We now define the truth function, $v$, using the same recursive structure that we used to define a wff of SL.

HUN: Most meghatározzuk az igazságfüggvényt ($v$), ugyanazt a rekúrzív struktúrát használva, amit a wff SL meghatározásánál is használtunk.
...
HUN:
\kezdődik{felsorolás}
\tétel Ha \forgatókönyv{A} egy mondat betűje, akkor $v(\forgatókönyv{A})=a(\forgatókönyv{A})$.
...
HUN: %Ez egy matematikai egyenlőségjel, nem az egyéni állítmány amit meghatároztunk a QL-nek.
...
\tétel Ha \forgatókönyv{A} van ${\enot}\forgatókönyv{B}$ néhány mondat számára \forgatókönyv{B}, akkor
\kezdődik{matematikai változó kijelzése}v(\forgatókönyv{A}) =
	\bal\{\kezdődik{tömb}{ll}
	1 & \mbox{ha $v(\forgatókönyv{B}) = 0$},\\
	0 & \mbox{egyébként.}
	\vége{tömb}\jobb.
\vége{matematikai változó kijelzése}

\tétel Ha \forgatókönyv{A} van $(\forgatókönyv{B}\eand\forgatókönyv{C})$ néhány mondat számára \forgatókönyv{B,C}, akkor
\kezdődik{matematikai változó kijelzése}v(\forgatókönyv{A}) =
	\bal\{\kezdődik{tömb}{ll}
	1 & \mbox{ha $v(\forgatókönyv{B}) = 1$ és $v(\forgatókönyv{C}) = 1$,}\\
	0 & \mbox{egyébként.}
	\vége{tömb}\jobb.
\vége{matematikai változó kijelzése}
\számláló beállítása{Példa}{\arab{enumi}}\vége{felsorolás}
...
HUN: Úgy tűnhet, hogy ez a meghatározás körkörös, mivel használja az `és` szót miközben megpróbálja meghatározni azt. Megjegyzendő, hogy ez nem az angol `és` szó meghatározása; ez az igazságtartalmú mondatok meghatározása melyek az `\eand.` logikai szimbólumtartalmú SL-hez tartoznak. Az igazságot olyan tárgyi nyelv számára határozzuk meg, amely tartalmazza az `\eand` szimbólumot és `and` metanyelvet használ. Itt semmilyen körkörös folyamat nincs.
...
%...
\kezdődik{felsorolás}\számláló beállítása{enumi}{\arab{Példa}}
\tétel Ha \forgatókönyv{A} van $(\forgatókönyv{B}\eor\forgatókönyv{C})$ néhány mondat számára \forgatókönyv{B,C}, akkor
\kezdődik{matematikai változó kijelzése}v(\forgatókönyv{A}) =
	\bal\{\kezdődik{tömb}{ll}
	0 & \mbox{van $v(\forgatókönyv{B}) = 0$ és $v(\forgatókönyv{C}) = 0$,}\\
	1 & \mbox{egyébként.}
	\vége{tömb}\jobb.
\vége{matematikai változó kijelzése}
%\számláló beállítása{Példa}{\arab{enumi}}\vége{felsorolás}
...
%...
%\kezdődik{felsorolás}\számláló beállítása{enumi}{\arab{Példa}}
\tétel Ha \forgatókönyv{A} van $(\forgatókönyv{B}\eif\forgatókönyv{C})$ néhány mondat számára \forgatókönyv{B,C}, akkor
\kezdődik{matematikai változó kijelzése}v(\forgatókönyv{A}) =
	\bal\{\kezdődik{tömb}{ll}
	0 & \mbox{ha $v(\forgatókönyv{B}) = 1$ és $v(\forgatókönyv{C}) = 0$,}\\
	1 & \mbox{egyébként.}
	\vége{tömb}\jobb.
\vége{matematikai változó kijelzése}
